...
def adjust_columns(wb):
    ...
    for ws in wb.worksheets:
        ...
        for row in ws.iter_rows():
            for cell in row:
                col_letter = get_column_letter(cell.column)  # Utiliser get_column_letter pour obtenir la lettre de la colonne
                if cell.value is not None:
                    cell_length = len(str(cell.value))
                else:
                    cell_length = 0
                if col_letter not in max_lengths or cell_length > max_lengths[col_letter]:
        ...
        for col_letter, length in max_lengths.items():
            ...
            ws.column_dimensions[col_letter].width = max(10, adjusted_width)  # Largeur minimale de 10 pour éviter trop de réduction
