def process_esp_D(esp_row, ws, current_row):
    ...
    code_esp = code_esp.text if code_esp is not None and code_esp.text is not None else ''
    if code_esp :
    ...
    nom_vern = nom_vern.text if nom_vern is not None and nom_vern.text is not None else '' 
           
    statut_bio_esp = esp_row.find('.//STATUT_BIO_ESP')    
    statut_bio_esp = statut_bio_esp.text if statut_bio_esp is not None and statut_bio_esp.text is not None else '' 
    ...
    espece_values = [groupe, code_esp, nom_esp, nom_vern, statut_bio_esp_txt, source, deg_abd_txt, eff_I, eff_S, observation]
